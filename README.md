...
### 卷积数据交互
#### CIP数据
在接收到权重加载或计算使能信号后依次启动相应子模块，并在完成后释放busy信号，从而实现对第一层卷积的完整控制。同时，内部通过 sipo_ram 将串行输入的激活值转换为并行数据，并在主时钟域下通过状态机逐地址写入第二层卷积的RAM。顶层模块直接将这些输出端口引出供外部使用。
#### CIM数据
首先将指定层的权重从cim_weight加载到外部S10 SRAM，然后依次启动各层计算，每层计算时通过S10接口串行输出激活地址、激活数据、权重及控制信号（如s10_addr、s10_weight、s10_compute_en），与外部CIM阵列交互完成乘累加。计算过程中，卷积层的输出结果会通过cim_max_pool模块进行最大池化降维，并自动作为下一层的输入激活写入对应RAM，全连接层计算完成后输出分类结果。
...
权重电路： 
10个3×3的矩阵控制前30列，尾端3x3的矩阵。其中有效的的权重为3×32个序列。  
...
#### 模拟电路
conv2层： 分为Inputdirver、SRAM、Comparator部分。Inputdriver部分接收FPGA写入的激活值，SRAM中90×32构成filter部分，Comparator部分比较每一列的电流得到三元值。

conv3层：分为Inputdirver、SRAM、Comparator部分。Inputdriver部分接收FPGA写入的激活值，SRAM中288×32构成filter部分，Comparator部分比较每一列的电流得到三元值。

FC层： 分为Inputdirver、SRAM、Outselect部分。Inputdriver部分接收FPGA写入的激活值，由于数据已经经过展平，SRAM中1152×10构成filter部分，Outselect通过比较10列输出10个模拟数字值作为分累结果。
