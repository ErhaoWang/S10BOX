...
### 卷积数据交互
#### CIP数据
该模块 cip_weight 作为数字与模拟电路的接口，负责存储卷积层的二值权重（通过主时钟写入双端口RAM），并在加载使能后，通过状态机控制以串行方式依次输出每个通道的权重矩阵及根据权重和映射得到的ccap配置码，同时生成相应的同步控制信号（如右移时钟、输出使能），从而为模拟计算单元提供所需的配置数据流。
#### CIM数据
